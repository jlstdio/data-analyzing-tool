...
class cifar10_expanded_2_dataloader(object):
    ...
    def get_rotate_data_1(self):
        ...
        x_train_rot = self._random_rotate_range(self.x_train, -30, 30)
        x_test_rot = self._random_rotate_range(self.x_test, -30, 30)
    ...
    def get_rotate_data_2(self):
        ...
        if self.x_train is None:
            raise ValueError("Please call load_data() first.")
        x_train_rot = self._random_rotate_range(self.x_train, 60, 120)
        x_test_rot = self._random_rotate_range(self.x_test, 60, 120)
        return (x_train_rot, self.y_train.copy()), (x_test_rot, self.y_test.copy())
    ...
    def get_rotate_data_3(self):
        ...
        if self.x_train is None:
            raise ValueError("Please call load_data() first.")
        x_train_rot = self._random_rotate_range(self.x_train, 150, 210)
        x_test_rot = self._random_rotate_range(self.x_test, 150, 210)
        return (x_train_rot, self.y_train.copy()), (x_test_rot, self.y_test.copy())
    ...
    def get_rotate_data_4(self):
        ...
        if self.x_train is None:
            raise ValueError("Please call load_data() first.")
        x_train_rot = self._random_rotate_range(self.x_train, 240, 310)
        x_test_rot = self._random_rotate_range(self.x_test, 240, 310)
        return (x_train_rot, self.y_train.copy()), (x_test_rot, self.y_test.copy())
    ...
    def _color_emphasis(self, images, mode='red'):
        ...
        if mode == 'red':
            # R 채널만 좀 더 키우기
            scale = np.array([1.7, 1.0, 1.0])
        elif mode == 'green':
            scale = np.array([1.0, 1.5, 1.0])
        elif mode == 'orange':
            # 주황색: 빨강+초록 계열
            scale = np.array([1.5, 1.4, 0.8])
        elif mode == 'purple':
            # 보라색: 빨강+파랑 계열
            scale = np.array([1.3, 0.8, 1.3])
        else:
            # 기본값은 변경없음
    ...
    def _apply_frequency_filter(self, images, filter_type='lowpass'):
        ...
        # 필터 파라미터 (32x32 기준)
        radius1 = 2   # 대략 저주파/고주파 경계
        radius2 = 6  # bandpass/bandstop 범위
...
if __name__ == "__main__":
    # 1) 원본 데이터 로드
    dataloader = cifar10_expanded_2_dataloader('./cifar10_expanded_2', normalize=True)
